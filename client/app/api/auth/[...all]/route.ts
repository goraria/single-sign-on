...
async function createRedirectResponse(request: Request, response: Response) {
  ...
  let payload: BetterAuthRedirectPayload

  try {
    payload = (await response.clone().json()) as BetterAuthRedirectPayload
  } catch {
    return null
  }
  ...
  if (!payload.redirect || !redirectUrl) return null
...
async function proxyAuth(
  ...
  const targetUrl = new URL(`/auth/${all.join("/")}`, ssoServerUrl)
    ...
    return Response.json(
      {
        ...
        message: `Cannot connect to SSO server at ${ssoServerUrl}`,
      },
  ...
  if (redirectResponse) {
    return logProxyResponse(request, all.join("/"), redirectResponse)
  }
