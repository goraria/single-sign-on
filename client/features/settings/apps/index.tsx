...
export function Apps() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <ul className="grid min-h-0 flex-1 gap-4 pt-4 pb-16 ring-0 md:grid-cols-3">
        {filteredApps.map(([name, Icon, connected, description]) => (
          <Card key={name} className="gap-2 px-6 hover:shadow-md">
            <div className="mb-4 flex items-center justify-between">
              <div className="bg-muted flex size-9 items-center justify-center rounded-lg p-2">
                <Icon className="size-4" />
              </div>
              <Button variant={connected ? "default" : "outline"}>
