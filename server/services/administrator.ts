...
import {
  administratorSsoApplicationPatchSchema,
  administratorSsoApplicationPayloadSchema,
  type AdministratorSsoApplicationPatch,
  type AdministratorSsoApplicationPayload,
} from "@/schemas/administrator"
...
function parseAdministratorPayload(input: unknown) {
  try {
    return administratorSsoApplicationPayloadSchema.parse(input)
  } catch (error) {
...
function parseAdministratorPatch(input: unknown) {
  try {
    return administratorSsoApplicationPatchSchema.parse(input)
  } catch (error) {
    if (error instanceof z.ZodError) {
      throw createServiceError("invalid_payload", 400)
    }
...
function getPostLogoutRedirectUris(
  body: AdministratorSsoApplicationPayload,
  redirectUris: string[]
) {
...
function buildCreateValues(input: unknown) {
  const body = parseAdministratorPayload(input)
...
export type AdministratorSsoApplication = Awaited<
  ReturnType<typeof listSsoApplications>
>[number]
export type {
  AdministratorSsoApplicationPatch,
  AdministratorSsoApplicationPayload,
}
